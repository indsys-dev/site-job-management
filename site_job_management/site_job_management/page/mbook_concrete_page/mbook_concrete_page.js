frappe.pages['mbook-concrete-page'].on_page_load = function(wrapper) {
    render_mbook_dashboard(wrapper, "M-Book Concrete Dashboard",
        "site_job_management.site_job_management.page.mbook_concrete_page.mbook_concrete_page.get_all_mbook_concrete_data",
        "quantity"
    );
};
        ...
        callback: function(r) {
            ...
            Object.keys(grouped).forEach(report_no => {
                ...
                grouped[report_no].forEach(row => {
                    ...
                    let bg = index % 2 === 0 ? "#e0f7fa" : "#f1f9f9"; // cool alternating rows
                    rows += `
                ...
                html += `
                <div style="margin-bottom:20px; background:white; border-radius:12px; border:1px solid #cfd8dc; box-shadow:0 4px 10px rgba(0,0,0,0.08);">
                    <div style="padding:12px 15px; font-weight:bold; display:flex; justify-content:space-between;
                                background: linear-gradient(90deg, #ffffff, #ffffff); color:black; border-top-left-radius:12px; border-top-right-radius:12px;">
                        <span>📌 ${report_no}</span>
